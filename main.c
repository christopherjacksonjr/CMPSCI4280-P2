...
//void displayResults(vector<Token> tokens);
//string determineTokenID(int enumID);
...
int main(int argc, char* argv[])
{
	...
	{	
		...
		//Calling scanner function then displaying results.
	...
	if(argc == 2)
	{
		...
		else
		{
			...
			//Calling scanner function and displaying results.
			tokens = scanner(buffer);
			parser(tokens);
...
/*void displayResults(vector<Token> tokens)
{
	...
	for (int x = 0; x != tokens.size(); ++x)
        {
		tokenID = determineTokenID(tokens[x].id);		
...
//Function that takes in token enumID and returns corresponding string.
string determineTokenID(int enumID)
{
	switch(enumID)
	{
